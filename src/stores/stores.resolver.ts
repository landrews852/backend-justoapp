...
export class StoresResolver {
  ...
  async updateStore(@Args("input") store: UpdateStoreInput) {
    console.log(store);
    return await this.storesService.update(store);
  }
  ...
  async store(@Args("input") input: FindStoreInput) {
    if (input._id) {
      return await this.storesService.findById(input._id);
    }
    if (input.name) {
      return await this.storesService.findOne(input.name);
    } else
      return Error(
  ...
  async storeHistory(@Parent() parent: Store) {
    return await this.historyService.findByRelationId(parent._id);
  }
}
